...
class TDAgent():
    ...
    def learn(self):
        ...
        states_np = np.asarray(batch.state, dtype=np.float32)
        states = torch.as_tensor(states_np, dtype=torch.float32, device=self.device)  # [bs, input_dim]
        ...
        non_final_next_states_np = np.asarray(
        ...
        next_state_values_ = torch.zeros(self.batch_size, 1, device=self.device)
